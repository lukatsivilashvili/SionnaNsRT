...
cat "$latest/coord_000_"*/raw/ns3_to_tap_topology.json
...
`tap0` is created once by the Python runner and reused with ns-3 `TapBridge` in `UseLocal` mode. This prevents `tap0` from disappearing between per-coordinate ns-3 runs.
...
TCP note: tcpdump can show packets arriving on `tap0`. For a completed TCP data transfer into the Linux host TCP stack, run a host listener during the simulation:
